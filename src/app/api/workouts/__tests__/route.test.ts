...
describe("POST /api/workouts", () => {
  ...
  it("returns 400 when exercises exceed max (50)", async () => {
    ...
    const res = await POST(makePostRequest({ ...validPayload, exercises }));
    expect(res.status).toBe(400);
    const data = await res.json();
  ...
  it("returns 400 for duration out of range (-1)", async () => {
    const res = await POST(makePostRequest({ ...validPayload, duration_minutes: -1 }));
    expect(res.status).toBe(400);
    const data = await res.json();
  ...
  it("returns 400 for duration out of range (1000)", async () => {
    const res = await POST(makePostRequest({ ...validPayload, duration_minutes: 1000 }));
    expect(res.status).toBe(400);
    const data = await res.json();
    expect(data.error).toMatch(/duration/i);
  });
  ...
  it("returns 400 for body_weight of 0", async () => {
    const res = await POST(makePostRequest({ ...validPayload, body_weight: 0 }));
    expect(res.status).toBe(400);
    const data = await res.json();
  ...
  it("returns 400 for body_weight out of range (1000)", async () => {
    const res = await POST(makePostRequest({ ...validPayload, body_weight: 1000 }));
    expect(res.status).toBe(400);
    const data = await res.json();
    expect(data.error).toMatch(/body weight/i);
  });
